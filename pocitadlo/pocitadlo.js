...
function alertSkore()
{
		...
		var spr = "Bohužiaľ nesprávny výsledok, Vaše aktuálne skóre je:\n\n"+spravnych+" správnych príkladov";
		var trv = (isNaN(trvanie) || trvanie < 0)?"":(" za\n"+trvanie+" sekúnd");
		var prie = (isNaN(priemer) || priemer < 0)?"":(", čo je\n"+priemer+" sekúnd na príklad v priemere.");
		var tip = (isNaN(priemer) || priemer < 0) || priemer > 5?"\n\nTip: pre vynulovanie času po prečítaní tejto správy stlačte Enter alebo kliknite na 'Resetovať', po napísaní výsledku stlačte Enter alebo kliknite na 'Je to správne?' a po potvrdení správnosti výsledku stlačte Enter alebo kliknite na 'Nový príklad' - pri použití klávesy Enter namiesto klikania môžete výrazne skrátiť Váš priemerný čas.":(priemer < 3)?"\n\nVýborne!":"\n\nPočítať, počítať, počítať!";
